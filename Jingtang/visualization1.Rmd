...
## Adjacency matrix and Reording metrics
...
1. **Nodes**:
   - The nodes are listed on both the rows and columns, suggesting a square matrix. Each unique node is represented by a unique identifier (e.g., 219045, 239470, etc.).

2. **Connections**:
   - Each cell in the matrix represents the connection between the node corresponding to its row and the node corresponding to its column.
   - A black cell indicates a connection between the two nodes, while an absence of a black cell (white space) indicates no connection.

Analysis of the Matrix:

1. **Diagonal**:
   - The matrix does not show any self-loops, as indicated by the absence of black cells along the diagonal. This means no node is connected to itself, which is typical in many social network representations.

2. **Symmetry**:
   - The matrix appears to be asymmetric, which suggests that the network might be directed. This means that if node A is connected to node B, it doesn’t necessarily imply that node B is connected back to node A.

3. **Density**:
   - The density of connections varies throughout the matrix. There are clusters of densely connected nodes, which could indicate groups or communities within the music network.
   - For example, the lower left corner and the central part of the matrix show more dense connections, suggesting closely-knit groups of nodes.

4. **Isolated Nodes**:
   - Some nodes have fewer connections, shown by fewer black cells in their corresponding rows and columns. These could represent individuals or entities that are less connected within the network.

5. **Clusters**:
   - The dense blocks of connections may represent subgroups within the music network. These could be groups of musicians, genres, or communities that are more interconnected.

Practical Implications:

- **Community Detection**:
  - The dense clusters can be investigated further to identify communities or groups that have a high level of interaction.

- **Central Nodes**:
  - Nodes with many connections (rows and columns with many black cells) can be considered central or influential within the network.

- **Network Analysis**:
  - This matrix provides a foundation for applying various network analysis techniques, such as calculating centrality measures or detecting communities, to understand the structure and dynamics of the network.

In summary, this adjacency matrix offers a visual summary of the connections within a social network related to music. It highlights densely connected groups and less connected individuals, which can be crucial for understanding the network's structure and dynamics.
...
##### based on community
Since the fast greedy algorithm we learned cannot be directly applied to our directed network, I opted to experiment with various algorithms. I applied the walktrap algorithm to our original network and simultaneously converted the original network into an undirected one. Then, I employed the fast greedy method to the undirected version. Additionally, I also considered the optimal community detection alogrithm that we used previously. Finally,I compare their modularity scores and observe differences in communities and reordered matrices. 
...
```{r}
# Create the adjacency matrix
adj_matrix <- t(as_adjacency_matrix(book.network, sparse = FALSE))
...
```{r}
modularity_value <- modularity(book.cluster)
modularity_value

membership <- membership(book.cluster)
...
# Plot the reordered adjacency matrix
image(t(ordered_adjacency_matrix), main="Reordered Adjacency Matrix for Book(optimal)", xlab="Nodes", ylab="Nodes", axes=FALSE, col=c("white", "black"))
...
# Reorder the nodes based on community membership
ordered_indices <- order(membership)
ordered_adjacency_matrix <- t(as.matrix(get.adjacency(undirected_book_network))[ordered_indices, ordered_indices])

# Plot the reordered adjacency matrix
image(ordered_adjacency_matrix, main="Reordered Adjacency Matrix for Book(fastgreedy)", xlab="Nodes", ylab="Nodes", axes=FALSE, col=c("white", "black"))
